...
DATA_SPLITS_FILE = ROOT_DIR / "metadata/data_splits.json"
USER_NAME = "admin"

LABELS_FIELD_NAME = "labels_created"

# in demo mode, we do not persist user state changes
demo_mode = False
...
def add_to_seen_list(image_id: str, user_id: str = USER_NAME):
    if demo_mode:
        print("Demo mode active - not persisting seen list changes")
        return
    if USERS_STATE_FILE.exists():
        with open(USERS_STATE_FILE, "r", encoding="utf-8") as f:
            users_state = json.load(f)
...
def add_to_labels_list(image_id: str, label_vector: list, user_id: str = USER_NAME):
    ...
    user_state[LABELS_FIELD_NAME] = labels_list
    users_state[user_id] = user_state
...
_all_image_paths = sorted(list(PAINTINGS_DIR.glob("*.jpg")))
...
_image_size_order_cache = {}
def get_image_size_order_cache():
    global _image_size_order_cache
    if not _image_size_order_cache:
        size_id_pairs = []
        for path in _all_image_paths:
            image_id = path.stem.split("_")[0]
            size = path.stat().st_size
            size_id_pairs.append((size, image_id))
        # Sort by size descending
        size_id_pairs.sort(reverse=True, key=lambda x: x[0])
        for index, (_, image_id) in enumerate(size_id_pairs):
            _image_size_order_cache[image_id] = index
    return _image_size_order_cache


def get_random_image_id(num: int = 1, exclude: list = [], exclude_test: bool = True, exclude_train: bool = False, prioritize_large_files: bool = False):
    """
    Get random image ID(s) that haven't been seen yet.
    """
    print(f"Getting random image ID with exclude_test={exclude_test}, exclude_train={exclude_train}")
    if exclude_test and exclude_train:
        raise ValueError("Cannot exclude both test and train sets.")
    
    if exclude_test or exclude_train:
        if DATA_SPLITS_FILE.exists():
            with open(DATA_SPLITS_FILE, "r", encoding="utf-8") as f:
                data_splits = json.load(f)
        else:
            raise FileNotFoundError("Data splits file does not exist.")
        
        if exclude_test:
            exclude += data_splits.get("test", [])
        if exclude_train:
            exclude += data_splits.get("train", [])

    ids_in_folder = [p.stem.split("_")[0] for p in _all_image_paths]
    ...
    if prioritize_large_files:
        possible_ids.sort(key=lambda x: get_image_size_order_cache().get(x, float('inf')))
        possible_ids = possible_ids[:len(possible_ids)//10]
